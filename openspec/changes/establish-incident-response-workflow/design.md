...
- **`ErrorCluster`** — a deduplicated group of `LogEvent`s sharing a signature: `signature`, `count`, `first_seen`, `last_seen`, `sample_message`, `severity`. **[Implemented, corrected]** What actually reaches Gemini is the `Incident` (which embeds this `ErrorCluster`) plus exactly one bounded, representative `LogEvent` — never an unbounded or per-occurrence raw log stream (see `backend/tracy/gemini.py`'s `build_prompt`). The `severity` field here uses `LogEvent`'s log-level scale (`CRITICAL`/`ERROR`/...), not `incident-package.schema.json`'s business-impact scale (`critical`/`high`/...) — see `detection.py`'s `map_severity` for the deterministic translation; it is not a direct mirror for that one field.
